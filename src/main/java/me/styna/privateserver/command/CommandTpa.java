...
public class CommandTpa extends BasePrivateCommand {

    public CommandTpa(PrivateServer mod) {
        super(mod);
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        TeleportService.RequestResult result = mod.getTeleportService().requestTpa(requester, target, false, tick);
        switch (result) {
            case SELF:
            ...
            default:
                send(requester, mod.getConfigService().getTeleportConfig().commandPrefix,
                        "&aRequest sent to &e" + target.getName() + "&a. Use /tpdeny or /tpaccept.");
                send(target, mod.getConfigService().getTeleportConfig().commandPrefix,
                        "&e" + requester.getName() + " &awants to teleport to you. Use &e/tpaccept &aor &e/tpdeny&a.");
        }
    }
